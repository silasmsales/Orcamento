...
$(document).ready(function(){
  ...
  $('#editarentradas').on('show.bs.modal', function (event) {
...
<div class="modal" tabindex="-1" role="dialog" id="editarentradas">
  <div class="modal-dialog" role="document">
    <form action="{{route('entradas.atualizar')}}" method="post">
...
<div class="modal" tabindex="-1" role="dialog" id="novoentrada">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
...
<section class="content-header">
  ...
  <ol class="breadcrumb">
    ...
    <li class="active"><a href="#"><i class="fa fa-dashboard"></i>Entradas</a></li>
  </ol>
</section>
...
<section class="content">
  <div class="row">
    ...
    <div class="col-md">
        ...
        <div class="box-body no-padding">
          <table class="table table-striped">
            ...
            <tr>
              ...
              <td>{{$entrada->id}}</td>
              <td>{{$entrada->conta->descricao}}</td>
              <td>{{$entrada->valor}}</td>
              <td>{{$entrada->data}}</td>
              <td>{{$entrada->descricao}}</td>
              <td>
                <div class="btn-group">
                  <button data-toggle="modal" data-target="#editarentradas" data-id="{{$entrada->id}}" data-conta_id="{{$entrada->conta_id}}" data-valor="{{$entrada->valor}}" data-data="{{$entrada->data}}" data-descricao="{{$entrada->descricao}}" data-descricao_detalhada="{{$entrada->descricao_detalhada}}" type="button" class="btn btn-info">Editar</button>
                  <a  class="btn btn-danger" href="javascript:confirm('Deletar  {{$entrada->id." - ".$entrada->descricao}}?') ? window.location.href='{{ route('entradas.deletar', $entrada->id) }}': false">Deletar</a>
                </div>
              </td>
            </tr>
            @endforeach
      ...
      <div class="btn-group">
        <button data-toggle="modal" data-target="#novoentrada" type="button" class="btn btn-block btn-success">Nova entrada</button>
      </div>
      <!-- /.box -->
    </div>
    <!-- /.col -->
